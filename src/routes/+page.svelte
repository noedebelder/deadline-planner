<script>
  ...
  function tageVerbleibend(deadlineStr) {
    ...
    const diff = Math.ceil((deadline - heute) / (1000 * 60 * 60 * 24));
    return diff;
  }
      ...
      <tbody>
        {#each data.deadlines as d}
          <tr class={statusKlasse(d.deadline)}>
            ...
            <td><span class="status {d.status || 'offen'}">{d.status || 'Offen'}</span></td>
            <td>
              <div class="aktionen">
...
<style>
  ...
  .stat-zahl { font-size: 2rem; font-weight: 700; }
  ...
  tr.ueberfaellig { background: #fff5f5; }
  tr.kritisch { background: #fff8f0; }
  tr.warnung { background: #fffdf0; }
  ...
  .badge.rot { background: #fde; color: #e74c3c; }
  .badge.orange, .badge.kritisch { background: #fef3e2; color: #f39c12; }
  ...
  .prio.hoch { background: #fde; color: #e74c3c; }
  ...
  .status.offen { background: #eef2ff; color: #5c6bc0; }
  .status.in-bearbeitung { background: #fef3e2; color: #f39c12; }
  .status.erledigt { background: #eafaf1; color: #27ae60; }

  .aktionen { display: flex; gap: 0.4rem; }
  .btn-edit, .btn-delete {
  ...
  .btn-delete:hover { background: #fde; }
